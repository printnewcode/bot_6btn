...
def forward_check(message):
    ...
    markup.add(btn)
    bot.send_message(message.chat.id, text_7, reply_markup=markup)
...
def callback_r(call):
    if call.data == "btn_1":
        ...
        bot.send_media_group(call.message.chat.id, [
            telebot.types.InputMediaPhoto(open(os.path.join('media', 'second.JPG'), 'rb')),
            telebot.types.InputMediaPhoto(open(os.path.join('media', 'second_2.JPG'), 'rb'))
            ]
            )

        btn = types.InlineKeyboardButton(text="Программа NOVAя INTENSIVE", callback_data='btn_2')
        markup.add(btn)
        bot.send_message(call.message.chat.id, text_1, reply_markup=markup)

    elif call.data == "btn_2":
        markup = types.InlineKeyboardMarkup()
        btn = types.InlineKeyboardButton(text="Что нужно для тренировок?", callback_data='btn_3')
        markup.add(btn)
        bot.send_message(call.message.chat.id, text_2, reply_markup=markup)

    elif call.data == "btn_3":
        markup = types.InlineKeyboardMarkup()
        btn = types.InlineKeyboardButton(text="Хочу попасть на интенсив", callback_data='btn_4')
        markup.add(btn)
        bot.send_message(call.message.chat.id, text_3, reply_markup=markup)
        ...
        markup.add(btn)
        bot.send_message(call.message.chat.id, text_4, reply_markup=markup)

    elif call.data == "btn_5":
        markup = types.InlineKeyboardMarkup()
        btn = types.InlineKeyboardButton(text="Да, готова", callback_data='btn_6')
        markup.add(btn)
        bot.send_message(call.message.chat.id, text_5, reply_markup=markup)

    elif call.data == "btn_6":
        msg = bot.send_message(call.message.chat.id, text_6)
        ...
        markup.add(btn)
        bot.send_message(call.message.chat.id, f'Наш менеджер {manager} поможет вам и ответит на ваши вопросы', reply_markup=markup)
